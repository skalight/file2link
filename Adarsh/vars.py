...
class Var(object):
    ...
    BOT_TOKEN = str(getenv('BOT_TOKEN', '5397695914:AAGOQT0lD37ilkl7ihAovNEvee8XeZQQhf8'))
    SESSION_NAME = str(getenv('SESSION_NAME', 'filetoink'))
    ...
    if 'DYNO' in environ:
        ON_HEROKU = True
        APP_NAME = str(getenv('APP_NAME', 'filetoink'))
    
    else:
        ON_HEROKU = False
    FQDN = str(getenv('FQDN', '15.207.143.189')) if not ON_HEROKU or getenv('FQDN') else APP_NAME+'.herokuapp.com'
    ...
    UPDATES_CHANNEL = str(getenv('UPDATES_CHANNEL', "@TamilMob_LinkZz"))
